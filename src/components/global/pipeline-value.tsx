...
const PipelineValue = ({ subaccountId }: Props) => {
		...
		if (pipelines.length) {
			return (
				pipelines
					...
					?.Lane?.reduce(
						(totalLanes, lane, currentLaneIndex, array) => {
	...
	const pipelineRate = useMemo(
		() =>
			(pipelineClosedValue / (totalPipelineValue + pipelineClosedValue)) *
			100,
	...
	return (
		<Card className="relative w-full xl:w-[350px]">
			...
			<CardContent className="text-sm text-muted-foreground">
				<p className="mb-2">
					Total value of all tickets in the given pipeline except the
					last lane. Your last lane is considered your closing lane in
					every pipeline.
				</p>
				<Select
